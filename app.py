...
# СУПЕР-АГРЕССИВНОЕ ОТКЛЮЧЕНИЕ КЭШИРОВАНИЯ ДЛЯ ВСЕХ РЕСУРСОВ
@app.after_request
def disable_all_cache(response):
    """Полностью отключает кэширование для HTML, CSS, JS и всех статических файлов"""
    # Агрессивные заголовки против кэширования
    response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate, max-age=0, private'
    response.headers['Pragma'] = 'no-cache'
    response.headers['Expires'] = '0'
    response.headers['Last-Modified'] = '0'
    response.headers['ETag'] = ''
    # Заставляем браузер перезагружать все
    response.headers['X-Accel-Expires'] = '0'
    return response

# Добавляем timestamp для версионирования статических файлов
import time
CACHE_BUSTER = int(time.time())

@app.context_processor
def inject_cache_buster():
    """Добавляет уникальный timestamp к CSS/JS файлам для полного обхода кэша"""
...
def view_presentation(unique_id):
    ...
    for prop in presentation.properties:
        ...
        if property_data:
            ...
            # Parse coordinates - check multiple sources
            ...
            # Also check coordinates from property_info if available
            ...
            enriched_property = {
                ...
                'renovation_display_name': property_data.get('renovation_display_name') or (property_detail.renovation_display_name if property_detail else None),
...
def manager_dashboard():
    ...
    try:
        ...
        # Prevent caching - добавляем заголовки против кэширования
...
def manager_presentation_view(presentation_id):
    """View individual presentation page with property cards"""
        ...
        if property_data:
            ...
            if isinstance(property_id, str):
                try:
                    ...
                    print(f"DEBUG: Invalid property ID: {property_id}")
                    continue
            ...
            enriched_property = {
                ...
                'title': property_data.get('title', f"{property_data.get('rooms', 0)}-к.кв."),
                'rooms': property_data.get('rooms', 0),
                'price': property_data.get('price', 0),
                'area': property_data.get('area', 0),
                'floor': property_data.get('floor', 0),
                'total_floors': property_data.get('total_floors', 0),
                'complex_name': property_data.get('residential_complex', 'Не указан'),
                'property_type': property_data.get('property_type', 'Квартира'),
                'main_image': property_data.get('main_image', '/static/images/no-photo.jpg'),
                'layout_image': property_data.get('main_image', '/static/images/no-photo.jpg'),
                ...
                'price_per_sqm': property_data.get('price_per_sqm', 0),
                'status': property_data.get('status', 'available'),
                'renovation_display_name': property_data.get('renovation_display_name', ''),
                'balcony_count': property_data.get('balcony_count', 0),
                'loggia_count': property_data.get('loggia_count', 0)
            }
            enriched_properties.append(enriched_property)
        else:
    ...
    # Update view count
    try:
        presentation.view_count = (presentation.view_count or 0) + 1
        presentation.last_viewed_at = db.func.now()
        db.session.commit()
    except Exception as e:
        print(f"DEBUG: Error updating view count: {e}")
...
def get_manager_presentations():
    """Получить все презентации менеджера с объектами недвижимости"""
    from models import Collection, CollectionProperty
    from flask_login import current_user
    ...
    presentations_data = []
    properties_data = load_properties()  # Загружаем все объекты недвижимости
    
    for presentation in presentations:
        # Получаем объекты недвижимости для этой презентации
        collection_properties = CollectionProperty.query.filter_by(
            collection_id=presentation.id
        ).order_by(CollectionProperty.order_index).all()
        
        # Обогащаем объекты данными из load_properties
        enriched_properties = []
        for cp in collection_properties:
            ...
            if property_data:
                ...
                enriched_property = {
                    ...
                    'property_type': property_data.get('property_type', 'Квартира'),
                    'main_image': property_data.get('main_image', '/static/images/no-photo.jpg'),
                    'layout_image': property_data.get('main_image', '/static/images/no-photo.jpg'),  # Используем главное фото как планировку
                    'address': property_data.get('address', ''),
                    'developer': property_data.get('developer', 'Не указан'),
                    'district': property_data.get('district', 'Не указан'),
                    'cashback': property_data.get('cashback', 0),
        ...
        presentation_dict = presentation.to_dict()
        presentation_dict['properties'] = enriched_properties
        presentations_data.append(presentation_dict)
...
def property_download_pdf(presentation_id, property_id):
    ...
    try:
        ...
        # Загрузить полные данные объекта из базы
        from models import ExcelProperty
        import json
        
        # Получить объект из базы данных
        excel_property = ExcelProperty.query.filter(
            (ExcelProperty.inner_id == int(property_id)) if property_id.isdigit() else 
            (ExcelProperty.inner_id == property_id)
        ).first()
        
        if not excel_property:
            abort(404)
        
        # Подготовить фотографии (парсим JSON один раз)
        photos_list = json.loads(excel_property.photos) if excel_property.photos else []
        
        # Подготовить данные с обогащением из базы
        property_data = {
            'id': excel_property.inner_id,
            'name': f"{'Студия' if excel_property.object_rooms == 0 else f'{excel_property.object_rooms}-к. квартира'}, {excel_property.object_area} м²",
            'title': f"{'Студия' if excel_property.object_rooms == 0 else f'{excel_property.object_rooms}-к. квартира'}, {excel_property.object_area} м²",
            'price': excel_property.price or 0,
            'price_per_sqm': excel_property.square_price or 0,
            'area': excel_property.object_area or 0,
            'rooms': excel_property.object_rooms or 0,
            'floor': excel_property.object_min_floor or 1,
            'total_floors': excel_property.object_max_floor or excel_property.object_min_floor or 1,
            'address': excel_property.address_display_name or 'Адрес уточняется',
            'latitude': excel_property.address_position_lat,
            'longitude': excel_property.address_position_lon,
            'complex_name': excel_property.complex_name or 'Не указан',
            'developer_name': excel_property.developer_name or 'Не указан',
            
            # ОТДЕЛКА - реальные данные из базы
            'renovation': excel_property.renovation_display_name or 'Не указана',
            
            # ФОТО ОБЪЕКТА - реальные данные из базы
            'photos': photos_list,
            'images': photos_list,
            
            # ПОЭТАЖНЫЙ ПЛАН - первое фото для первой страницы PDF
            'layout_image': photos_list[0] if photos_list else None,
            
            # СРОК СДАЧИ - реальные данные из базы
            'completion_quarter': excel_property.complex_building_end_build_quarter,
            'completion_year': excel_property.complex_building_end_build_year,
            'completion_date': f"{excel_property.complex_building_end_build_quarter} кв. {excel_property.complex_building_end_build_year} г." if excel_property.complex_building_end_build_quarter and excel_property.complex_building_end_build_year else "Уточняется",
            
            # ДАТА СТАРТА ПРОДАЖ - реальные данные из базы
            'sales_start_quarter': excel_property.complex_start_build_quarter,
            'sales_start_year': excel_property.complex_start_build_year,
            'sales_start_date': f"{excel_property.complex_start_build_quarter} кв. {excel_property.complex_start_build_year} г." if excel_property.complex_start_build_quarter and excel_property.complex_start_build_year else "Уточняется",
        ...
        # Подготовить данные менеджера
        from datetime import datetime
        current_date = datetime.now().strftime('%d.%m.%Y в %H:%M')
        
        # Получить данные менеджера
        manager_data = None
        if presentation.created_by:
            from models import Manager
            # Если created_by уже объект Manager, используем его ID
            manager_id = presentation.created_by.id if hasattr(presentation.created_by, 'id') else presentation.created_by
            manager_data = Manager.query.get(manager_id)
        
        # Превратить словарь в объект для шаблона
        from types import SimpleNamespace
        property_obj = SimpleNamespace(**property_data) if isinstance(property_data, dict) else property_data
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                if excel_property.address_display_name:
                    ...
                    excel_property.parsed_country = parsed.get('country', 'Россия')
                    excel_property.parsed_region = parsed.get('region', '')
                    excel_property.parsed_city = parsed.get('city', '')
                    excel_property.parsed_district = parsed.get('district', '')
                    excel_property.parsed_street = parsed.get('street', '')
                    excel_property.parsed_house_number = parsed.get('house_number', '')
